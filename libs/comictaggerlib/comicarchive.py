...
class ZipArchiver:
	...
	def getArchiveFilenameList( self ):
		try:
			zf = zipfile.ZipFile( self.path, 'r' )
			namelist = []
			for ni in zf.namelist():
				if not isinstance(ni, unicode):
					tryanother=False


					try:
						namelist.append(ni.decode('utf-8'))
					except UnicodeDecodeError as e:
						print e
						tryanother = True

					if tryanother:
						namelist.append(ni.decode('windows-1252'))
				else:
					namelist.append(ni)
			zf.close()
			return namelist
		...
		while tries < 7:
			try:
				...
				for item in rarc.infolist():
					if item.size != 0:
						if not isinstance(item.filename, unicode):
							tryanother=False


							try:
								namelist.append(item.filename.decode('utf-8'))
							except UnicodeDecodeError:
								tryanother = True

							if tryanother:
								namelist.append(item.filename.decode('windows-1252'))
						else:
